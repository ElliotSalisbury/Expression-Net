...
file = np.load("./fpn_new_model/perturb_Oxford_train_imgs_mean.npz", fix_imports=True, encoding='bytes')
train_mean_vec = file["train_mean_vec"]  # [0,1]
del file
file = np.load("./fpn_new_model/perturb_Oxford_train_labels_mean_std.npz", fix_imports=True, encoding='bytes')
mean_labels = file["mean_labels"]
std_labels = file["std_labels"]
del file

# Get training image mean for Shape CNN
mean_image_shape = np.load('./Shape_Model/3DMM_shape_mean.npy', fix_imports=True, encoding='bytes')  # 3 x 224 x 224
mean_image_shape = np.transpose(mean_image_shape, [1, 2, 0])  # 224 x 224 x 3, [0,255]

# Get training image mean for Expression CNN
mean_image_exp = np.load('./Expression_Model/3DMM_expr_mean.npy', fix_imports=True, encoding='bytes')  # 3 x 224 x 224
...
def extract_PSE_feats():
    ...
    # Face Pose-Net
    ###################
    net_data = np.load("./fpn_new_model/PAM_frontal_ALexNet.npy", fix_imports=True, encoding='bytes').item()
    net_data = convert(net_data)
    ...
    with tf.variable_scope('shapeCNN'):
        ...
        npzfile = np.load('./ResNet/ShapeNet_fc_weights.npz', fix_imports=True, encoding='bytes')
    ...
    with tf.variable_scope('exprCNN'):
        ...
        npzfile = np.load('./ResNet/ExpNet_fc_weights.npz', fix_imports=True, encoding='bytes')
